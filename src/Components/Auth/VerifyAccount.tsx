...
import { useResendVeifyOtp, useVerify_Otp } from "@/Hooks/api/auth_api";
...
export default function VerifyAccount() {
  ...
  const { mutate: verifyOtp, isPending: isVerifying } = useVerify_Otp();
  const { mutate: resendOtp, isPending: isResending } = useResendVeifyOtp();
  ...
  const handlePaste = (e: React.ClipboardEvent<HTMLDivElement>) => {
    ...
    if (paste) {
      ...
      if (paste.length >= 4) inputRefs.current[3]?.blur();
    }
  };
  ...
  const handleResend = () => {
    resendOtp({ email }, { onSuccess: () => setTimeLeft(RESEND_COOLDOWN) });
  };
      ...
      <Container>
        <div className="flex h-screen items-center justify-center">
          <p className="text-red-500 text-xl">
            Invalid access. Please go back to the login page.
          </p>
        </div>
      </Container>
    );
        ...
        <div className="w-full xl:w-[35%] lg:w-[50%] flex items-center justify-center px-6">
          <div className="w-full">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <p className="text-gray-500 text-center lg:text-start">
                ...
                <button
                  ...
                  className="text-primary-blue font-medium hover:underline disabled:text-gray-400 disabled:no-underline disabled:cursor-not-allowed"
                >
                  {isResending
                    ? "Sending..."
                    : timeLeft > 0
              ...
              <button
                ...
                className="w-full bg-primary-blue text-white py-3 rounded-lg hover:bg-blue-600 transition disabled:opacity-70 disabled:cursor-not-allowed cursor-pointer"
              >
                {isVerifying ? (
            ...
            <Link href="/auth/login">
              <button className="flex gap-x-2 items-center text-[20px] font-medium text-[#0085FF] mx-auto cursor-pointer mt-8">
